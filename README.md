...
## Content packs (JSON)
Weapons, spells, monsters, presets, and traits are all defined the same way: drop a JSON file in the matching subfolder of `<world folder>/dndsheets/` and it loads automatically on server start (or immediately with the matching `/dnd... load <filename>` command). You don't need to copy anything to get started: the default packs (87 spells, 330 monsters, 362 magic items, weapons, presets, traits) are seeded into a fresh world's `dndsheets/` folder on first server start. `/test/dndsheets` in this repo holds one minimal `ejemplo.json` per content type as a schema reference — the shortest correct example of each format.

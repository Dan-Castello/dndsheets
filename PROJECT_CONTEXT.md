...
14. **The Guide read as a reference card, not as onboarding — and part of it was invisible.** A pass over it from the player's side, not the code's, found four things at once. (a) A vanilla written book **silently truncates** anything past 14 lines of 114px (`BookViewScreen`, `128 / 9`); `page.level_up` was 704 characters, so over half of it had never been on screen and nothing in the game said so. `GuideBook` now measures with the same splitter vanilla draws with and splits a long page across as many book pages as it needs, and `checkPatchouliBook` caps every page at 320 characters because **Patchouli cannot** split — its page clips the same way and it is the one reader nothing can fix at runtime. (b) Without Patchouli the book was a flat strip of 31 pages you could only read by turning them one at a time: it is now cut into the same categories and entries as the Patchouli book, behind an index whose rows are `ClickEvent.CHANGE_PAGE` links (vanilla resolves those itself — no new screen), with a link back on every page. `checkPatchouliBook` now compares the **grouping and order** of the two, not just that they hold the same pages, since from here on they can drift while both still "contain everything". (c) Three shipped subsystems appeared nowhere in it — the compendium, the campaign journal and magic-item attunement — and the quick command reference listed 14 of the 20 registered commands. (d) Worst for a first-time player: the opening page said "press H" and never named `CharacterSetupScreen`, the one screen that turns a blank sheet into a playable character. It leads now. (e) The deepest one, and the reason the other four were worth fixing: the Guide taught the *mod* and never *D&D* — it said AC, saving throw and advantage without ever defining them, and the word "campaign" did not appear in it once. A **"Primera partida"** category now sits in front of everything else (what a DM is, 1d20 + modifier vs. a difficulty, advantage, the four words you will hear, your first ten minutes, your first turn), plus a "Tu primera sesión como DM" entry leading the DM category. It is the only part of the book that can be read by someone who has never played, so it is the part that is read first. The Guide went from 31 pages / 1.246 words to 54 pages across 22 entries.

15. **Half the mod was hardcoded in one language, and there was no way to tell which half.** Reported as "the book is hardcoded in English"; the book was fine, but the *reason* it looked that way was real and bigger. Two separate failures, both invisible from the language files: (a) **the roll editor was never translated** — eight keys sat in `es_es.json` holding their English text (`Roll Expression`, `Insert Modifiers`, `Main Roll`, `Extra Roll`, `Context`, `Attack Roll`, `Bonus`), so a Spanish player got an English screen while `checkLanguageFiles` reported both languages complete, because it compares *keys*, never *values*; (b) **48 `Component.literal` sites held Spanish prose** — every class-resource item's name and tooltip (Tótem de Furia, Ayudar, Vara de DM, Segundo Aliento…) plus a batch of command replies, so an English player read the mod in Spanish and grepping the language files for the string found nothing, because it was never in them. `checkChatMessagesAreTranslatable` only guarded `sendSystemMessage(Component.literal("`, which is why none of it ever failed the build. All 50 moved to keys; the check now also fails on **any** literal carrying prose (a lowercase letter followed by a space) outside `command/`, and caps the 86 that remain inside it so they cannot grow. Deliberately left alone: the `STR`/`DEX`/`Prof.` buttons, because the button writes `$str`/`$prof` into the expression box and a label that does not match what it writes is worse than an untranslated one. Along the way the Guide was telling Spanish players to use tabs named "Main, Skills and Attacks" — the tabs had been translated (Principal/Habilidades/Ataques) and the Guide had not noticed.

16. **The whole mod showed in English to a Spanish player, and the language files were perfect.** Reported as "the Guide is in English with the game in Spanish", with a screenshot where vanilla's own page counter read *"Página 4 de 86"* — so the locale was definitely Spanish. `es_es.json` was valid, complete, all-strings, no BOM, and present in the jar. The cause: **Minecraft has no regional fallback.** It loads `en_us` and then the *exact* language selected, and 1.20.1 ships **seven** Spanish locales (`es_es`, `es_ar`, `es_cl`, `es_ec`, `es_mx`, `es_uy`, `es_ve`). A player on "Español (México)" gets vanilla's `es_mx` and, from a mod that only ships `es_es`, **`en_us`** — game in Spanish, mod in English, nothing in the log, and nothing wrong to find in the language files, because the file that failed was the one that did not exist. There is no way to declare "`es_*` uses `es_es`": the file has to exist under each name. `tools/sync_lang_variants.py` copies it to the other six and `checkLanguageFiles` fails if any copy drifts from the original. Worth remembering as a class of bug, not a one-off: the same trap is waiting for `pt_br`/`pt_pt`, `zh_cn`/`zh_tw` and every other regional pair the day someone translates this.
17. **NPCs could not do anything by themselves, and never appeared in the party.** Everything the mod spawns was a vanilla mob with `NoAI:1` — deliberate (the DM drives it with the wand, and `MonsterActionManager.moveTowardIfNeeded` *simulates* an approach rather than pathing), but it also meant `baseEntity` pointing at an NPC mod's entity produced a body whose own objectives — patrol, follow the party, hold a post — were killed one line after it spawned. `"ai": true` on the monster JSON (and `/dndchar spawn <id> <entity> true` for a character-backed NPC) now keeps them, defaulting to `false` so no existing pack changes behaviour. The load-bearing half is in `TurnManager.freeze`, whose gate changed from "is it a mob with no stat block?" to **"is this mob's AI on?"** — those were the same question only while the mod's own creatures were always `NoAI`, and a monster that keeps its AI has to be frozen between turns like any other or it walks through the encounter out of turn. So the rule is: the AI is for *outside* combat; inside it, `autoAct` and 5e still decide. `sendParty` also walks the loaded entities now, so NPCs with a body show up in the DM's party roster with their real HP, AC and conditions.

    The follow-up request was "let me transfer a created creature onto an Easy NPC entity, that mod is far better at controlling them" — and reading Easy NPC's source (MIT, public `de.markusbordihn.easynpc.api` package, entities registered as `easy_npc:humanoid`, `easy_npc:villager`, … from `DefaultNPCType`) showed the integration was **the wrong shape**. Spawning an Easy NPC *from* this mod would mean a soft dependency, their preset format, and a bridge to maintain — to arrive at a creature this mod configured worse than their own UI does. The direction that costs nothing is the opposite one: **`/dndmonsters bind <target> <id>` attaches a stat block to a creature that already exists.** The DM builds it wherever it is best built and then declares what it is; from that moment `Combatant.of` resolves it like any bestiary monster, because the binding is the same persistent NBT tag `spawnAt` was already writing. No dependency on Easy NPC, and it works identically on a vanilla mob or any other mod's — which is exactly why no integration was needed. It deliberately leaves the AI alone (that creature came configured from somewhere) and does not overwrite a custom name, but it does set HP from the block: inheriting the villager's 20 HP for a 52 HP captain is not "kept state", it is a wrong stat block. Gear is applied **only for the slots the block declares** — `equip` already ignored blank ones — so a block that only names a sword gives it the sword and leaves the armour someone dressed it in.

    The follow-up was "make it an item and a screen: click the NPC, pick the mob, done". No new item was needed: the DM Wand's right-click on a creature **with no stat block** did nothing at all, which is a free interaction sitting on the tool the DM already carries for exactly this kind of job. It now opens the picker. The list is built from `MonsterRegistry.ids()` **on the client**, like `MonsterSpawnListScreen` — the registry is already there, so opening the screen costs no round trip, and the only thing that has to travel is *which* creature. One `MonsterBindMessage` carries both directions (invariant 3): server→client with an empty `monsterId` means "open the picker for this entity", client→server with an id means "apply it". Both entry points — command and picker — run the same `MonsterRegistry.applyStatBlock`, because two ways of doing the same thing have to do the same thing. Sneak+click still deletes **only** what the mod spawned (plus practice armor stands): a creature that has not been given a sheet yet is not ours to delete, and one stray sneak-click would otherwise destroy an NPC someone had just finished building.

18. **Sneak+click with the DM Wand never deleted an armor stand — and never had.** The handler was correct, the branch was right there, and the event simply never arrived. The cause is two lines of vanilla, in two different classes: `ArmorStand.interactAt` returns `CONSUME` **the moment it sees it is on the client** — before it looks at the item, the slot or anything else, so that right-click can equip the stand — and `Minecraft.startUseItem` only retries with `interact` *if the first result did not consume* (`if (!interactionresult.consumesAction())`). So the INTERACT packet is never sent and `PlayerInteractEvent.EntityInteract` never fires for an armor stand; the only event that arrives is `EntityInteractSpecific`, from INTERACT_AT. Fixed by subscribing to that one as well, **filtered to armor stands**: every other entity sends both packets (its `interactAt` returns PASS and the client retries), so handling both events unfiltered would run the handler twice per click — the same double-pass failure `InteractionEvents` documents, arriving through a different door. Pinned in `checkInteractHandlers`, because this is the kind of root cause you cannot find by reading this mod, only by reading vanilla.

19. **Wild Shape became real, and the answer was not to port a morph mod.** The ask was "druids should actually transform", pointing at iChun's Morph as something to port. Measured, that was the wrong door: Morph is **1.16.5** (four Minecraft versions back, across the 1.17 rendering rewrite), **LGPL v3** against this repo's MIT, depends on **iChunUtil**, and carries **10 mixin classes** including `PlayerRendererMixin` — while `checkPortabilityCoupling` fails this build if a mixin or access transformer appears at all. And 110 files of "kill a mob, collect it, morph GUI" to obtain one thing: rendering a player as another entity. Forge fires `RenderPlayerEvent.Pre` for exactly that, so the visual half is one cancel plus one `EntityRenderDispatcher.render` call with **zero** offsets — the event fires inside the renderer, after the dispatcher already pushed and translated onto the player, so passing the position again would draw the beast at twice the distance from the camera.

    The rules half is the part worth reading. The obvious design was a `WildShapeCombatant`, and it was wrong: every question the engine asks — "what is its AC", "how many hit points are left", "what is its Strength modifier" — already routes through the sheet and Minecraft's health attribute. So transforming **writes the beast's numbers there** and stashes the druid's underneath, and the whole engine (a monster deciding whether it hits, the DM's party roster, the HUD, concentration) sees the form without a single one of those paths knowing the feature exists. A new combatant would have meant teaching every one of them to recognise it. The state lives on the **sheet**, not in memory, for the same reason conditions do: disconnecting mid-form must not leave anyone with a bear's Strength forever. The beast list is the bestiary filtered by `CreatureType.BEAST` — 95 of the 330 — so a content pack that adds beasts offers them with no list to maintain. Dropping to 0 in form reverts instead of downing, via a `HIGHEST`-priority `LivingDeathEvent` that cancels before `DeathSaveManager` sees it. `checkWildShape` compares the **whole sheet JSON** before and after a round trip rather than field by field, and its second case is the one that actually corrupts: a druid whose DM had already pinned an AC by hand: if the form does not distinguish "there was no override" from "there was one", the beast's AC stays nailed on after reverting. Deliberately not done: no CR limit, because the bestiary carries no CR field.

20. **Bosses that do not queue.** Asked as "some legendary mobs should ignore turns, keeping damage and effects" — and the architecture already had the seam. `TurnManager.tryAct` opens with `if (isIncapacitated(actor)) return false;` and *only then* asks whose turn it is: the rules gate and the clock gate were already two different lines. And 5e damage hangs off `CombatManager.onLivingHurt`, a `LivingHurtEvent`, which has never known the turn order exists. So `"ownClock": true` is "no queue", not "no rules": the creature leaves the ORDER but stays in `order` — take it out of the list and `allEnemiesDefeated` ends the encounter the moment the last minion dies, with the dragon still breathing. It acts every **120 ticks = 6 s = one 5e round**, which is the guardrail that keeps it fair: the same action economy, desynchronised, not unlimited. Three traps worth remembering: the skip in `advance` must go through `step()` or a round ending on the boss stops counting (walls, weapon buffs and summons all tick per round); `autoAct` must **not** reach `tryAct` even indirectly, because that schedules the auto-advance and would hand someone else's turn away every six seconds; and legendary actions are forced to zero, since they are 5e's patch for the very problem this removes. Opportunity attacks are excluded both ways — something that never stops moving would otherwise trigger them continuously. Aggro is nailed down by walking `goalSelector.getAvailableGoals()` and dropping `PanicGoal`/`AvoidEntityGoal` (a burning ravager fleeing ends the encounter) plus `setPersistenceRequired()`, and re-targeting each cycle. The table gets a vanilla title/subtitle packet with a line per creature type, because a dragon moving out of turn with no explanation reads as a bug, not as design. Shipped marked on the 14 apex bosses (ten ancient dragons, tarrasque, lich, kraken, solar); the adult dragons and sphinxes keep legendary actions, so both systems ship live. **Not playtested at a table yet** — the feel is the one thing no self-test measures.

...
- `TomeButton` (`client/gui/components/TomeButton.java`) — fila y botón con la identidad del mod: tira de pergamino sobre cuero, filete de latón a la izquierda y biselado de Minecraft. **Sustituye al botón gris de vanilla en todo el mod.** Solo hay tres sitios que fabrican botones (`ListPickerScreen.addRow`, `SmallFormScreen`, `ModalDialogScreen.addModalButton`), así que cambiarlo ahí repinta por dentro las más de cuarenta pantallas. El foco se marca por **dos** vías —fondo y filete encendido, más texto aclarado— porque un solo cambio de tono sobre fondo oscuro no se distingue con brillo bajo. El texto se centra en el hueco que queda tras el filete, no en el botón entero, o quedaría descuadrado.
